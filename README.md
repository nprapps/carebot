...
```
cd carebot
mkvirtualenv --no-site-packages carebot
...
```
* * * * * ubuntu bash /home/ubuntu/apps/carebot/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test 
```
...
|--------|------|-----|-----|--------|--------|
|carebot|tweet|`location`||||
|carebot|facebook|`location`||||
|carebot|email|`location`||||
|carebot|new-comment||||
|carebot|open-share-discuss||||
|carebot|close-share-discuss||||
|carebot|summary-copied||||
|carebot|featured-tweet-action|`action`||``tweet_url``|
|carebot|featured-facebook-action|`action`||``post_url``|

...
app.register_blueprint(static.static, prefix='/%s' % app_config.PROJECT_SLUG)
...
class Project(db.Model):
    ...
    def __unicode__(self):
        return u'%s Carebot Project' % self.title
...
class ProjectAdmin(ModelView):
    column_filters = ['property_id', 'domain']
    form_excluded_columns = ['slug']

    column_sortable_list = ('title', 'property_id', 'domain', 'prefix', 'start_date')

    form_args = dict(
        property_id = dict(default = '53470309'),
        domain = dict(default = 'apps.npr.org')
    )

    def update_model(self, form, model):
        model.slug = slugify([form.data['title']])
...
class QueryAdmin(ModelView):
    ...
    def update_model(self, form, model):
        ...
        super(QueryAdmin, self).update_model(form, model)
